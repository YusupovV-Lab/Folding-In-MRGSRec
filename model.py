...
class MRGSRecModel(nn.Module):
    ...
    def _apply_graph_encoder(self, ind = None):
        ...
        if ind is not None:
            ego_embeddings = torch.cat((ego_embeddings[:ind], self._newuser_embeddings.weight, ego_embeddings[ind + 1:]), dim=0)
    ...
    def forward(self, inputs, ind = None):
        ...
        all_final_user_embeddings, all_final_item_embeddings = \
            self._apply_graph_encoder(ind)  # (num_users + 2, embedding_dim), (num_items + 2, embedding_dim)
